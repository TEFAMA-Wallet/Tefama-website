...
import { LayoutGrid, Gauge, ChartLine, Radio, ListChecks, OctagonX, Bell, Wallet } from "lucide-react";
...
const FEATURES = [
  ...
  { Icon: ChartLine, h: "Portfolio performance dashboard", b: "Track portfolio ROI, agent performance, daily returns, and risk metrics in one place. Drill into any agent's contribution." },
        ...
        <section className="section">
          <div className="container">
            <div className="grid-3">
              {FEATURES.map((f) => (
                <Card key={f.h} className="feature-card" style={{ padding: 32 }}>
                  <div className="feature-ico"><f.Icon size={24} /></div>
                  <h3 style={{ fontSize: 18, marginBottom: 10 }}>{f.h}</h3>
                  <p style={{ lineHeight: 1.7 }}>{f.b}</p>
                </Card>
              ))}
            </div>
          </div>
